...
def initialize_parameters():
    ...
    for line in lines:
        if not((str(line).strip()) and (line[0] == '#')): # IGNORA SE VAZIO OU COMENTARIO
            ...
            elif (parameter[0] == 'LAMBDA_ALPHA'):
                LAMBDA_ALPHA = float(parameter[1]) 
            elif (parameter[0] == 'N_OBJECTIVE'):
                N_OBJECTIVE = int(parameter[1])  
...
def execute_thread(solution_deliver, id_thread):
    ...
    objective1, objective2 = get_fitness(result_table)
    solution = (result_table, objective1, objective2)
    index = id_thread * N_OBJECTIVE
    with solution_pool.get_lock():
        solution_pool[index] = objective1
        solution_pool[index + 1] = objective2
    solutions = []
    ...
    for i in range(TEMPERATURE_LIST[5]):
        ...
        while ((current_temperature > TEMPERATURE_LIST[4])):
            ...
            for card in card_dict.items():
                ...
                objective1, objective2 = get_fitness(result_table)
                new_solution = (result_table, objective1, objective2)                
                
                # TEM OUTRO JEITO MAIS OTIMIZADO DE COPIAR ????????????????????????????
                # COPIA AS SOLUÇÕES CORRENTES DE TODAS AS THREADS LOCALMENTE
                copy = []
                with solution_pool.get_lock():
                    for i in range(N_OBJECTIVE * N_THREAD):
                        copy.append(solution_pool[i])

                # A PARTIR DE UMA SOLUÇÃO X, TENTA ENCONTRAR UMA SOLUÇÃO X' NA POOL QUE NÃO É DOMINADA POR X 
                dominated_test_result = True
                for i in range(N_THREAD):
                    if (i != id_thread):
                        index = i * N_OBJECTIVE
                        dominated_test_result = pg.pareto_dominance([solution[1], solution[2]], [copy[index], copy[index + 1]])
                    if (dominated_test_result == False):
                        break

                # SE NÃO ENCONTROU UM X' QUE NÃO É DOMINADO POR X, RANDOMIZA VALOR DOS PESOS, SENDO QUE A SOMA DEVE SER IGUAL A 1
                if dominated_test_result:
                    new_weight = np.random.dirichlet(np.ones(N_OBJECTIVE), size=1)
                    WEIGHT_LIST = []
                    for weight in new_weight:
                        WEIGHT_LIST.append(weight)
                # SE ENCONTROU UM X' QUE NÃO É DOMINADO POR X, ATUALIZA PESOS
                else:
                    for i in range(len(WEIGHT_LIST)):
                        #solution[i+1] = solution[i+1]
                        #new_solution[i+1] = new_solution[i+1]
                        if (solution[i + 1] >= copy[i + 1]):
                            WEIGHT_LIST[i] *= LAMBDA_ALPHA
                        else:
                            WEIGHT_LIST[i] /= LAMBDA_ALPHA
                            
                # ACEITA NOVA SOLUÇÃO COM CERTA PROBABILIDADE
                if (random.uniform(0, 1) <= acceptance_options[ACCEPTANCE_OPTION](solution, new_solution, current_temperature)):
                    solution = new_solution
                    solutions.append(solution)
                    # ATUALIZA A SOLUTION POOL DAS THREADS
                    index = id_thread * N_OBJECTIVE
                    with solution_pool.get_lock():
                        solution_pool[index] = objective1
                        solution_pool[index + 1] = objective2
                    ...
                    if (len(solutions) >= N_SOLUTION_POOL):
                        ...
                        solutions = []
                    # APENAS PRINT *********
                    if (new_solution[2] < 4000):
                        print('----------------' + str(new_solution[2]) + '----------------------------' + str(id_thread))
    ...
    if (id_thread == 0):
        for i in range(N_THREAD * N_OBJECTIVE): 
            pass
            #print(solution_pool[i])
    ...
    while (n_thread_cont > 0):
        ...
        if (message == 'thread_finished'):
            ...
            for solution in solutions:
                if (first_pass):
                    solutions_calculated = np.array([(solution[1], solution[2])])
                    first_pass = False
                else:
                    solutions_calculated = np.append(solutions_calculated, [(solution[1], solution[2])], axis=0)
            ...
            for index in new_solutions_index:
                ...
                new_solutions.append( (result_table, solutions[index][1], solutions[index][2]) )
            solutions = new_solutions
    print('RESULTADOS:')
    for solution in solutions:
        print('Valor: ' + str(solution[1]) + ' / Lojas: ' + str(solution[2]))
...
solution_pool = Array('d', range(N_THREAD * N_OBJECTIVE))
